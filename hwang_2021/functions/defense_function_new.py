...
def pgd_defense(discriminator, images, args, track_interm=False):
    ...
    for i in range(iters):
        ...
        outputs = discriminator(images, #ori_images, #images,
            images, #ori_images, #images,
            args).sum()

        grad = torch.autograd.grad(outputs, images,
                                   retain_graph=False, create_graph=False)[0]
        #zeros_in_grad = (grad.abs() < 1e-6).view(grad.shape[0], -1).sum(-1)
        #print(zeros_in_grad.max(), zeros_in_grad.min())
