...
class FeatureOutFCnet(nn.Module):
    def __init__(self, dim_y, DIM, depth, actlayer):
        ...
        self.block = [nn.Sequential(nn.Conv2d(dim_y, DIM, 1, 1, padding=0),
            nn.BatchNorm2d(DIM), actlayer(),
            )] # 1x1conv to 1x1 representation is same as fc followed by flatten.
...
class ConcatDiscriminator(nn.Module):
    def __init__(self, target_model, args):
        ...
        for i in range(args.after_concat_depth):
            ...
            self.block.append(nn.Sequential(nn.Conv2d(inter_dim * 2, inter_dim, 1, 1, padding=0),
    ...
    def forward(self, global_x, local_x, args):
        ...
        x1 = self.net1(global_x) if not args.layer1_off else torch.zeros([bs, 16, 1, 1], device=device)
        x2 = self.net2(global_x) if not args.layer2_off else torch.zeros([bs, 160, 1, 1], device=device)
        x3 = self.net3(local_x) if not args.layer3_off else torch.zeros([bs, 320, 1, 1], device=device)
        x4 = self.net4(local_x) if not args.layer4_off else torch.zeros([bs, 640, 1, 1], device=device)
