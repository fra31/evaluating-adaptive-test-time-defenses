...
def apgd_train(model, pfy_fn, x, y, norm='Linf', eps=8. / 255., n_iter=100,
    ...
    if not x_init is None:
        if isinstance(x_init, torch.Tensor):
            ...
            x_adv = get_init(x_init, model, x, y, eta=eps / 4, norm=norm,
                eps=eps)
    ...
    if norm in ['Linf', 'L2']:
        ...
        k = n_iter_2 + 0
        thr_decr = .75
        alpha = .25 if use_ge else 2. #.25 #2. #1e-5 #2e-4 #.01 #2.
    elif norm in ['L1']:
    ...
    if not use_interm:
        ...
        if not use_ge:
            for i_eot in range(eot_test + 1):
                ...
                if i_eot == 0:
                    logits = logits_step.detach().clone() # logits at current iterate (no smoothing)
...
def apgd_interm_restarts(full_def, model, pfy_fn, x, y, norm='Linf', eps=8. / 255., n_iter=10,
    ...
    log_path=None, early_stop=False, bpda_type=None, eot_test=0):
    ...
    for i in range(n_restarts):
        if acc.sum() > 0:
            ...
            acc_curr = full_def(x_adv_curr).max(1)[1] == y[acc]
